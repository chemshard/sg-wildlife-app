...
def get_wikipedia_extras(scientific_name, wikipedia_url=""):
    ...
    for sec in sections:
        heading = sec.get("line", "").lower().strip()
        anchor  = sec.get("anchor", "").lower().strip()
        combined = heading + " " + anchor

        if habitat_index is None:
            if any(h == heading or h == anchor for h in habitat_headings):
                habitat_index = sec.get("index")

        if feeding_index is None:
            if any(h == heading or h == anchor for h in feeding_headings):
                feeding_index = sec.get("index")
